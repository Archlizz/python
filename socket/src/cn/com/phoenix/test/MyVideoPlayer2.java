...
//			Format format = new Format(VideoFormat.H261);
			Vector<CaptureDeviceInfo> cdi = CaptureDeviceManager.getDeviceList(null);
			for(int i = 0; i < cdi.size(); i++){
			}
//			mediaLocator = cdi.get(0).getLocator();
//			mediaLocator = new MediaLocator("vfw://0");// 此类描述媒体目录的地址????
			String url2 = "vfw:Microsoft WDM Image Capture (Win64):0";  
//			CaptureDeviceInfo cdi = CaptureDeviceManager.getDevice(url2);
			URL url1 = new URL("file:/f:/heroe.mp3");
//			mediaLocator = new MediaLocator("url2");// 此类描述媒体目录的地址????
			audioLocator = new MediaLocator("javasound://44100");
//			mediaLocator = cdi.getLocator();
